...
function TasksBoardConsultant({ user, id }: TasksBoardProps) {
  ...
  const getProducerName = (
    producers: Producer[] | undefined, producerId: number
  ): string => {
    if(producers) {
  ...
  const getTasks = async () => {
    try {
      const response = await fetch(
        `/api/tasks/consultant?consultantID=${id}`, {
          method: 'GET',
          headers: {
  ...
  const fetchProducers = async () => {
    try {
      const response = await fetch(`/api/producers/search?withoutImages=${true}`, {
        method: 'GET',
      });
      if (!response.ok) {
      ...
      if (Array.isArray(data)) {
        ...
        throw new Error('Dados dos produtores inválidos.');
      }
    } catch(error) {
      console.log(error);
    }
      ...
      <div className="flex justify-between pl-3">
        <div className="flex flex-col">
          <p className="font-extrabold text-xl text-text-4">
            {tasks.length} tarefas de consultor
          </p>
                ...
                group.tasks.map((task, idx) => (
                  ...
                  >
                    <TaskItem
                      ...
                      producerName={
                        getProducerName(producers, task.producerId)
                      }
                    />
...
export default TasksBoardConsultant;
